...
export function BottomNav() {
  ...
  const items = [
    ...
    ...(!hasBusiness && !hasCompany
      ? [{ to: '/company', label: 'TEAMS', key: 'teams-pitch', icon: 'briefcase' as const }]
      : []),
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-paper border-t border-[rgba(11,16,24,0.08)] flex justify-around items-center py-3 px-4 z-50">
      {items.map((item) => {
        ...
        const showBriefcase = (item as { icon?: string }).icon === 'briefcase';
        return (
          <Link
            key={item.key}
            to={item.to}
            ...
            <span className="relative">
              {showBriefcase ? (
                <Briefcase
                  size={14}
                  strokeWidth={1.5}
                  className={isActive ? 'text-amber-brand' : 'text-neutral-gray'}
                />
              ) : (
                <span
